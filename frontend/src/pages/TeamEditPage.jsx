...
// ---- Delete Region Modal (with reassign option) ----
...
const TeamEditPage = () => {
  ...
  // Node modal state
  const [nodeModal, setNodeModal] = useState(null); // null | 'create' | {id, name, ...}

  // Region modal state
  const [regionModal, setRegionModal] = useState(null); // null | 'create' | {id, name, ...}
  const [deleteRegionModal, setDeleteRegionModal] = useState(null); // {region, nodes, targets}
  ...
  const filteredNodes = (nodes || []).filter(n => {
    ...
    const matchRegion = !regionFilter || (n.brainRegionId && String(n.brainRegionId) === regionFilter);
